...
impl Store {
    ...
    pub fn initialized(&self) -> bool {
        self.vault.keys.contains_key("default_identity")
    }
    pub fn get_identity(&self, name: &str) -> Option<&str> {
        self.vault
            .keys
            .get(&format!("{}_identity", name))
            .map(|s| s.as_str())
    }
    pub fn get_recipient(&self, name: &str) -> Option<&str> {
        self.vault
            .keys
            .get(&format!("{}_recipient", name))
            .map(|s| s.as_str())
    }
    ...
    pub fn store_keypair(&mut self, name: &str, identity: &str, recipient: &str) -> Result<()> {
        self.vault
            .keys
            .insert(format!("{}_identity", name), identity.into());
        self.vault
            .keys
            .insert(format!("{}_recipient", name), recipient.into());
        self.save()
    }
    ...
    fn save(&self) -> Result<()> {
        Ok(std::fs::write(
            self.path.join("vault.json"),
            serde_json::to_vec_pretty(&self.vault)?,
        )?)
    }
}
